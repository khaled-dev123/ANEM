...
class DynamicScorer:
    ...
    def __init__(self, overrides: dict[str, dict], meta: dict[str, dict] | None = None):
        self.overrides = overrides   # strategy → {C1..C6 normalised weights}
        self.meta      = meta or {}
    ...
    def from_cache(cls, path: Path = CACHE_PATH) -> "DynamicScorer":
        ...
        print(f"[DynamicScorer] Loaded ML overrides for strategies: "
    ...
    def from_mongo(cls, db) -> "DynamicScorer":
        ...
        if not overrides:
            ...
            print(f"[DynamicScorer] Loaded ML overrides for: {list(overrides.keys())}")
        return cls(overrides, meta)

    @classmethod
    def _base_only(cls) -> "DynamicScorer":
        """Fallback: use hand-tuned base weights for all strategies."""
        overrides = {s: get_normalized_weights(s) for s in STRATEGIES}
    ...
    def score(
        ...
        num   = sum(weights.get(k, 0) * scores[k] for k in scores)
        ...
        return {
            "strategy":            strat,
            "weights":             {k: round(weights.get(k, 0), 4) for k in scores},
    ...
    def score_resume(
        ...
        `offer` must contain: offre_ni, offre_diplome, offre_exp_years,
                              offre_metier, offre_lieu, date_offre.

        Returns the same schema as score(), plus `parsed_fields`.
        """
        record = {**offer, **parsed_resume}
        result = self.score(record, residence_scope=residence_scope)
    ...
    def print_override_summary(self) -> None:
        ...
        for strat in STRATEGIES:
            ...
            acc_str = f"  acc={m.get('accuracy', '?'):.3f}" if active and m.get("accuracy") else ""
